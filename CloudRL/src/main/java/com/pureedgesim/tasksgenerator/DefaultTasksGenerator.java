...
public class DefaultTasksGenerator extends TasksGenerator {
	...
	public List<Task> generate() {
		...
		SimLog.println("Simulation Time = " + simulationTime);
		...
		SimLog.println("Device Count which are generating Tasks=" + devices_count);

		// Browse all applications
		SimLog.println("Size of Application List" + SimulationParameters.APPLICATIONS_LIST.size());
		...
		for (int app = 0; app < SimulationParameters.APPLICATIONS_LIST.size() - 1; app++) {
			...
			SimLog.println("Usage % "+(int) SimulationParameters.APPLICATIONS_LIST.get(app).getUsagePercentage());
			SimLog.println("Devices Count " + devices_count);
			SimLog.println("Number of Devices " + numberOfDevices);
